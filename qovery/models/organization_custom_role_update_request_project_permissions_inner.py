...
class OrganizationCustomRoleUpdateRequestProjectPermissionsInner(BaseModel):
    ...
    permissions: Optional[List[OrganizationCustomRoleUpdateRequestProjectPermissionsInnerPermissionsInner]] = Field(default=None, description="Mandatory if `is_admin` is `false`   Should contain an entry for every environment type: - `DEVELOPMENT` - `PREVIEW` - `STAGING` - `PRODUCTION` ")
    __properties: ClassVar[List[str]] = ["project_id", "is_admin", "permissions"]
