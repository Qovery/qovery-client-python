...
class Application(BaseModel):
    ...
    maximum_cpu: Optional[StrictInt] = Field(None, description="Maximum cpu that can be allocated to the application based on organization cluster configuration. unit is millicores (m). 1000m = 1 cpu")
    maximum_memory: Optional[StrictInt] = Field(None, description="Maximum memory that can be allocated to the application based on organization cluster configuration. unit is MB. 1024 MB = 1GB")
