...
class DeploymentHistoryJobResponseAllOfSchedule(BaseModel):
    ...
    schedule_at: Optional[StrictStr] = Field(default=None, description="Can only be set if the event is CRON. Represent the cron format for the job schedule without seconds. For example: `* * * * *` represent the cron to launch the job every minute. See https://crontab.guru/ to WISIWIG interface. Timezone is UTC ")
    __properties: ClassVar[List[str]] = ["event", "schedule_at"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.schedule_at is None and "schedule_at" in self.model_fields_set:
