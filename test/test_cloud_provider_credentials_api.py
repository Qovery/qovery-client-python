# coding: utf-8

"""
    Qovery API

    - Qovery is the fastest way to deploy your full-stack apps on any Cloud provider. - ℹ️ The API is stable and still in development. 

    The version of the OpenAPI document: 1.0.3
    Contact: support+api+documentation@qovery.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from qovery.api.cloud_provider_credentials_api import CloudProviderCredentialsApi


class TestCloudProviderCredentialsApi(unittest.TestCase):
    """CloudProviderCredentialsApi unit test stubs"""

    def setUp(self) -> None:
        self.api = CloudProviderCredentialsApi()

    def tearDown(self) -> None:
        pass

    def test_create_aws_credentials(self) -> None:
        """Test case for create_aws_credentials

        Create AWS credentials set
        """
        pass

    def test_create_gcp_credentials(self) -> None:
        """Test case for create_gcp_credentials

        Create GCP credentials set
        """
        pass

    def test_create_scaleway_credentials(self) -> None:
        """Test case for create_scaleway_credentials

        Create Scaleway credentials set
        """
        pass

    def test_delete_aws_credentials(self) -> None:
        """Test case for delete_aws_credentials

        Delete a set of AWS credentials
        """
        pass

    def test_delete_gcp_credentials(self) -> None:
        """Test case for delete_gcp_credentials

        Delete a set of GCP credentials
        """
        pass

    def test_delete_scaleway_credentials(self) -> None:
        """Test case for delete_scaleway_credentials

        Delete a set of Scaleway credentials
        """
        pass

    def test_edit_aws_credentials(self) -> None:
        """Test case for edit_aws_credentials

        Edit a set of AWS credentials
        """
        pass

    def test_edit_gcp_credentials(self) -> None:
        """Test case for edit_gcp_credentials

        Edit a set of GCP credentials
        """
        pass

    def test_edit_scaleway_credentials(self) -> None:
        """Test case for edit_scaleway_credentials

        Edit a set of Scaleway credentials
        """
        pass

    def test_get_aws_credentials(self) -> None:
        """Test case for get_aws_credentials

        Get a set of AWS credentials
        """
        pass

    def test_get_gcp_credentials(self) -> None:
        """Test case for get_gcp_credentials

        Get a set of GCP credentials
        """
        pass

    def test_get_scaleway_credentials(self) -> None:
        """Test case for get_scaleway_credentials

        Get a set of Scaleway credentials
        """
        pass

    def test_list_aws_credentials(self) -> None:
        """Test case for list_aws_credentials

        List AWS credentials
        """
        pass

    def test_list_gcp_credentials(self) -> None:
        """Test case for list_gcp_credentials

        List GCP credentials
        """
        pass

    def test_list_scaleway_credentials(self) -> None:
        """Test case for list_scaleway_credentials

        List Scaleway credentials
        """
        pass


if __name__ == '__main__':
    unittest.main()
